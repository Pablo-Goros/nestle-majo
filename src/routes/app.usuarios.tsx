            ...
            {DEMO_USERS.map((u) => (
              <tr key={u.username} className="border-b last:border-0">
                ...
                <td className="py-2 px-2">
                  <button
                    disabled={!editable}
                    className="text-xs text-nestle-red font-medium disabled:opacity-50"
                  >
                    Editar
                  </button>
                </td>
                  ...
                  {SECCIONES.map((s) => {
                    ...
                    return (
                      <td key={s} className="py-2 px-2 text-center">
                        {edit ? (
                          <Badge tone="good">Editar</Badge>
                        ) : view ? (
                          <Badge tone="info">Ver</Badge>
                        ) : (
                          <Lock className="size-3 mx-auto text-muted-foreground" />
                        )}
                      </td>
                    );
                  })}
